    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#blastform-type').change(function() {
                ...
                $('select.type-switched-select option').not('.' + blasttype).hide();
                $('select.type-switched-select option.' + blasttype).show();
                
                $('select.type-switched-select').each(function(){
                    if (!$(this).children(':selected').is('.'+blasttype)){
                        $(this).children(':selected').prop('selected', false);
                        var vis = $(this).children('.'+blasttype);
                        vis.eq(0).prop('selected', true);
                    }
                }); 
        ...
        <div class="large-12 columns panel">
            ...
            <form id="blastform" method="POST" action="{#$webservice_job_start#}" data-goto="{#$result_page#}?jobid=">
                ...
                <ul class="large-block-grid-5">
                    ...
                    <li>
                        <label for="blastform-wordsize-blastn">Wordsize:</label>
                        <select id="blastform-wordsize-blastn" name="parameters[word_size]" style="width:auto"  class="type-switched-select">
                            <option class="blastn">11</option><option class="blastn">7</option><option class="blastn">15</option>
                            <option class="blastp blastx tblastn tblastx">3</option><option class="blastp blastx tblastn tblastx">2</option>
                        </select>
                    </li>
