...
(
  ...
  targetdb_identifier character varying NOT NULL,
  ...
  CONSTRAINT blast_cron_jobs_job_status_check CHECK (job_status::text = ANY (ARRAY['NOT PROCESSED'::character varying::text, 'PROCESSING'::character varying::text, 'PROCESSED'::character varying::text, 'ERROR'::character varying::text]))
)
WITH (
  OIDS=FALSE
);
ALTER TABLE blast_cron_jobs
  OWNER TO s202139;
