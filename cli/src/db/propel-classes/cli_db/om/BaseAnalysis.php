...
abstract class BaseAnalysis extends BaseObject implements Persistent
{
    ...
    /**
     * @var        PropelObjectCollection|Expressionresult[] Collection to store aggregation of Expressionresult objects.
     */
    protected $collExpressionresults;
    protected $collExpressionresultsPartial;
     ...
     * @var		PropelObjectCollection
     */
    protected $expressionresultsScheduledForDeletion = null;
    ...
    public function reload($deep = false, PropelPDO $con = null)
    {
        ...
        if ($deep) {  // also de-associate any related objects?
            ...
            $this->collExpressionresults = null;
    ...
    protected function doSave(PropelPDO $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->expressionresultsScheduledForDeletion !== null) {
                if (!$this->expressionresultsScheduledForDeletion->isEmpty()) {
                    foreach ($this->expressionresultsScheduledForDeletion as $expressionresult) {
                        // need to save related object because we set the relation to null
                        $expressionresult->save($con);
                    }
                    $this->expressionresultsScheduledForDeletion = null;
                }
            }

            if ($this->collExpressionresults !== null) {
                foreach ($this->collExpressionresults as $referrerFK) {
                    if (!$referrerFK->isDeleted() && ($referrerFK->isNew() || $referrerFK->isModified())) {
                        $affectedRows += $referrerFK->save($con);
                    }
                }
    ...
    public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collExpressionresults) {
                $result['Expressionresults'] = $this->collExpressionresults->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy && !$this->startCopy) {
            ...
            foreach ($this->getExpressionresults() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addExpressionresult($relObj->copy($deepCopy));
                }
            }
    ...
    public function initRelation($relationName)
    {
        ...
        if ('Expressionresult' == $relationName) {
            $this->initExpressionresults();
        }
    ...
    public function setAnalysisfeatures(PropelCollection $analysisfeatures, PropelPDO $con = null)
    {
        ...
        foreach ($analysisfeaturesToDelete as $analysisfeatureRemoved) {
            $analysisfeatureRemoved->setAnalysis(null);
        }
    ...
    protected function doAddAnalysisfeature($analysisfeature)
    {
        ...
        $analysisfeature->setAnalysis($this);
    }
    ...
    public function removeAnalysisfeature($analysisfeature)
    {
        if ($this->getAnalysisfeatures()->contains($analysisfeature)) {
            ...
            $analysisfeature->setAnalysis(null);
        }
     ...
     * @return PropelObjectCollection|Analysisfeature[] List of Analysisfeature objects
     */
    public function getAnalysisfeaturesJoinFeature($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = AnalysisfeatureQuery::create(null, $criteria);
    ...
    public function setAnalysisprops(PropelCollection $analysisprops, PropelPDO $con = null)
    {
        ...
        foreach ($analysispropsToDelete as $analysispropRemoved) {
            $analysispropRemoved->setAnalysis(null);
        }
     ...
     * @return PropelObjectCollection|Analysisprop[] List of Analysisprop objects
     */
    public function getAnalysispropsJoinCvterm($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
    {
        $query = AnalysispropQuery::create(null, $criteria);
    ...
    /**
     * Clears out the collExpressionresults collection
     *
     * This does not modify the database; however, it will remove any associated objects, causing
     * them to be refetched by subsequent calls to accessor method.
     *
     * @return Analysis The current object (for fluent API support)
     * @see        addExpressionresults()
     */
    public function clearExpressionresults()
    {
        $this->collExpressionresults = null; // important to set this to null since that means it is uninitialized
        $this->collExpressionresultsPartial = null;

        return $this;
    }

    /**
     * reset is the collExpressionresults collection loaded partially
     *
     * @return void
     */
    public function resetPartialExpressionresults($v = true)
    {
        $this->collExpressionresultsPartial = $v;
    }

    /**
     * Initializes the collExpressionresults collection.
     *
     * By default this just sets the collExpressionresults collection to an empty array (like clearcollExpressionresults());
     * however, you may wish to override this method in your stub class to provide setting appropriate
     * to your application -- for example, setting the initial array to the values stored in database.
     *
     * @param boolean $overrideExisting If set to true, the method call initializes
     ...
     * @return void
     */
    public function initExpressionresults($overrideExisting = true)
    {
        if (null !== $this->collExpressionresults && !$overrideExisting) {
            return;
        }
        $this->collExpressionresults = new PropelObjectCollection();
        $this->collExpressionresults->setModel('Expressionresult');
    }

    /**
     * Gets an array of Expressionresult objects which contain a foreign key that references this object.
     *
     * If the $criteria is not null, it is used to always fetch the results from the database.
     * Otherwise the results are fetched from the database the first time, then cached.
     * Next time the same method is called without $criteria, the cached collection is returned.
     ...
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param PropelPDO $con optional connection object
     * @return PropelObjectCollection|Expressionresult[] List of Expressionresult objects
     * @throws PropelException
     */
    public function getExpressionresults($criteria = null, PropelPDO $con = null)
    {
        $partial = $this->collExpressionresultsPartial && !$this->isNew();
        if (null === $this->collExpressionresults || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collExpressionresults) {
                // return empty collection
                $this->initExpressionresults();
            } else {
                $collExpressionresults = ExpressionresultQuery::create(null, $criteria)
                    ->filterByAnalysis($this)
                    ->find($con);
                if (null !== $criteria) {
                    if (false !== $this->collExpressionresultsPartial && count($collExpressionresults)) {
                      $this->initExpressionresults(false);

                      foreach($collExpressionresults as $obj) {
                        if (false == $this->collExpressionresults->contains($obj)) {
                          $this->collExpressionresults->append($obj);
                        }
                      }

                      $this->collExpressionresultsPartial = true;
                    }

                    $collExpressionresults->getInternalIterator()->rewind();
                    return $collExpressionresults;
                }

                if($partial && $this->collExpressionresults) {
                    foreach($this->collExpressionresults as $obj) {
                        if($obj->isNew()) {
                            $collExpressionresults[] = $obj;
                        }
                    }
                }

                $this->collExpressionresults = $collExpressionresults;
                $this->collExpressionresultsPartial = false;
            }
        }

        return $this->collExpressionresults;
    }

    /**
     * Sets a collection of Expressionresult objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param PropelCollection $expressionresults A Propel collection.
     * @param PropelPDO $con Optional connection object
     * @return Analysis The current object (for fluent API support)
     */
    public function setExpressionresults(PropelCollection $expressionresults, PropelPDO $con = null)
    {
        $expressionresultsToDelete = $this->getExpressionresults(new Criteria(), $con)->diff($expressionresults);

        $this->expressionresultsScheduledForDeletion = unserialize(serialize($expressionresultsToDelete));

        foreach ($expressionresultsToDelete as $expressionresultRemoved) {
            $expressionresultRemoved->setAnalysis(null);
        }

        $this->collExpressionresults = null;
        foreach ($expressionresults as $expressionresult) {
            $this->addExpressionresult($expressionresult);
        }

        $this->collExpressionresults = $expressionresults;
        $this->collExpressionresultsPartial = false;
    ...
    /**
     * Returns the number of related Expressionresult objects.
     *
     * @param Criteria $criteria
     * @param boolean $distinct
     * @param PropelPDO $con
     * @return int             Count of related Expressionresult objects.
     * @throws PropelException
     */
    public function countExpressionresults(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
    {
        $partial = $this->collExpressionresultsPartial && !$this->isNew();
        if (null === $this->collExpressionresults || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collExpressionresults) {
            ...
            if($partial && !$criteria) {
                return count($this->getExpressionresults());
            }
            $query = ExpressionresultQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collExpressionresults);
    }

    /**
     * Method called to associate a Expressionresult object to this object
     * through the Expressionresult foreign key attribute.
     *
     * @param    Expressionresult $l Expressionresult
     * @return Analysis The current object (for fluent API support)
     */
    public function addExpressionresult(Expressionresult $l)
    {
        if ($this->collExpressionresults === null) {
            $this->initExpressionresults();
            $this->collExpressionresultsPartial = true;
        }
        if (!in_array($l, $this->collExpressionresults->getArrayCopy(), true)) { // only add it if the **same** object is not already associated
            $this->doAddExpressionresult($l);
        }
    ...
    /**
     * @param	Expressionresult $expressionresult The expressionresult object to add.
     */
    protected function doAddExpressionresult($expressionresult)
    {
        $this->collExpressionresults[]= $expressionresult;
        $expressionresult->setAnalysis($this);
    }

    /**
     * @param	Expressionresult $expressionresult The expressionresult object to remove.
     * @return Analysis The current object (for fluent API support)
     */
    public function removeExpressionresult($expressionresult)
    {
        if ($this->getExpressionresults()->contains($expressionresult)) {
            $this->collExpressionresults->remove($this->collExpressionresults->search($expressionresult));
            if (null === $this->expressionresultsScheduledForDeletion) {
                $this->expressionresultsScheduledForDeletion = clone $this->collExpressionresults;
                $this->expressionresultsScheduledForDeletion->clear();
            }
            $this->expressionresultsScheduledForDeletion[]= $expressionresult;
            $expressionresult->setAnalysis(null);
        }
    ...
    public function clearAllReferences($deep = false)
    {
        if ($deep && !$this->alreadyInClearAllReferencesDeep) {
            ...
            if ($this->collExpressionresults) {
                foreach ($this->collExpressionresults as $o) {
                    $o->clearAllReferences($deep);
                }
            }
        ...
        $this->collAnalysisprops = null;
        if ($this->collExpressionresults instanceof PropelCollection) {
            $this->collExpressionresults->clearIterator();
        }
        $this->collExpressionresults = null;
